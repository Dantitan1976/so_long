...
void	ft_crear_mapa(t_juego *d)
{
	...
	textura->enemigo_text = mlx_load_png("textures/enemigo.png");
	...
	d->enemigo = mlx_texture_to_image(d->mlx, textura->enemigo_text);
	...
	mlx_delete_texture(textura->enemigo_text);
	free(textura);
}
...
void	ft_generar_enemigo(t_juego *d)
{
	int		col;
	int		fil;
	...
	while (d->mapa[fil])
	{
		...
		{
			if (d->mapa[fil][col] == 'X')
				mlx_image_to_window (d->mlx, d->enemigo, col * 75, fil * 75);
			col++;
		}
		col = 0;
		fil++;
